...
import { VizingXRouter, VizingXRouterV3 } from "../typechain-types";
...
export async function getVizingBridge() {
    const [owner, maker] = await ethers.getSigners();
    const bridge = await loadOrDeployContract<VizingXRouterV3>('VizingXRouterV3', false);
    return bridge;
}
describe("VizingBridge", function () {
    describe("Approve", function () {
        it("Maker TestToken Approve XVM", async function () {
            const [owner, maker] = await ethers.getSigners();
            const testToken = await getTokenContract();
            const totalSupply = await testToken.totalSupply();
            const VizingBridge = await getVizingBridge();
            const approveTx = await testToken.connect(maker).approve(VizingBridge.address, totalSupply);
        ...
        it("Check Maker TestToken Approve XVM Balance", async function () {
            const [owner, maker] = await ethers.getSigners();
            const testToken = await getTokenContract();
            const xvm = await getVizingBridge();
        ...
        it("User TestToken Approve XVM", async function () {
            ...
            const totalSupply = await testToken.totalSupply();
            const VizingBridge = await getVizingBridge();
            const approveTx = await testToken.connect(user).approve(VizingBridge.address, totalSupply);
            await approveTx.wait();
            expect(await approveTx.hash).not.empty;
        });
        it("Check User TestToken Approve XVM Balance", async function () {
            const [owner, _, user] = await ethers.getSigners();
            const testToken = await getTokenContract();
            const bridge = await getVizingBridge();
    ...
    describe("Deployment", function () {
        it("Deploy", async function () {
            const contract = await getVizingBridge();
